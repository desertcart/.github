...
class GitHubOrgAnalytics {
  constructor() {
    this.octokit = new Octokit({
      auth: `github_pat_${config.GITHUB_TOKEN}`,
    })
  ...
  async fetchOrganizationData() {
    try {
      ...
      const [orgInfo, repositories, members, recentActivity] =
        await Promise.all([
  ...
  async getRecentActivity() {
    const since = moment()
      .subtract(config.DAYS_FOR_RECENT_ACTIVITY, 'days')
      .toISOString()
    const activities = []
